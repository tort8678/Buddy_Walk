...
//   //----------------------------------------------------------------------
//   // 2/9/26 update: Upload to Firebase Storage
...
//   async function uploadBase64ImageToFirebase(base64Image: string) {
...
//             //----------------------------------------------------------------------
//             // 2/9/26 update
//             // Upload base64 image -> Firebase Storage, then store Firebase path in MongoDB
...
//                 firebaseImagePath = uploaded.path;
//               }
//             } catch (err) {
//               console.error(
...
//             //----------------------------------------------------------------------
//             // 2/9/26 update
//             // Upload base64 image to Firebase Storage, then store Firebase path in MongoDB, not base64
//             let firebaseImagePath = capturedImage as string;
...
export default function Test() {
  ...
  const startListening = (e: React.PointerEvent) => {
    if (!isListening) {
      ...
      recognitionRef.current?.start();
      // recognitionRef.current?.start?.(); // edit
    } else recognitionRef.current?.stop();
  };
  ...
  const stopListening = (e: React.PointerEvent) => {
    e.currentTarget.releasePointerCapture(e.pointerId);
    recognitionRef.current?.stop();
    // recognitionRef.current?.stop?.(); // edit
    setIsListening(false);
    // console.log(useSound('ui/item_deselect')); // Play sound when stopping listening
  };
  // -------------------------------------------------------------------------------------------------------------------
