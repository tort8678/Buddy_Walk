// main component that connects hooks (hooks section) and renders the UI (components section)
...
import CapturePhoto from "./components/CapturePhoto.tsx";
import QuestionSection from "./components/QuestionSection.tsx";
import ResponseSection from "./components/ResponseSection.tsx";
import { useSpeechSetup } from "./hooks/useSpeechSetup";
import { useMediaCapture } from "./hooks/useMediaCapture.ts";
import { useChatLog } from "./hooks/useChatLog.ts";
import { usePressToCapture } from "./hooks/usePressToCapture.ts";
...
export default function Test() {
  ...
  const { isListening, startListening, stopListening } =
    useSpeechSetup(setUserInput); // useSpeechSetup.ts
  ...
  const { currentChatId, currentMessageId, lastError, setLastError } =
    useChatLog({
      openAIResponse,
      userInput,
      image,
  ...
  const { handlePointerDown, handlePointerUp } = usePressToCapture({
    timeoutRef,
    startVideoRecording,
    stopVideoRecording,
    fileInputRef,
  ...
  const { loading, sendRequestOpenAI } = useOpenAI({
    userInput,
    image,
    videoBlob,
    coords: coords
  ...
  // -------------------------------------------------------------------------------------------------------------------
  // handleRetake NEEDS WORK - there is more functionality here, but very little logic in useMediaCapture
  // const handleRetake = () => {
  //   setVideoBlob(null);
  //   setImage(null);
  ...
  return (
    ...
    >
      ...
      <ResponseSection
        ...
        sendRequestOpenAI={sendRequestOpenAI}
        speak={speak}
...
// import { playSound, useSound } from "react-sounds";
// import { uploadBase64ImageToFirebase } from "../../api/firebaseUploadImg.ts";
...
//   //----------------------------------------------------------------------
//   // 2/27/26 update: Upload to Firebase Storage moved to firebaseUploadImg.ts
//   // Converts a dataURL ("data:image/jpeg;base64,...") into a Blob so that we can send it as multipart/form-data
...
//   //       //----------------------------------------------------------------------
//   //       // 2/9/26 update
//   //       // store the current image in a local variable before clearing state so that we can
//   //       // upload it and store its Firebase path in MongoDB
//   //       const capturedImage = image;
//   //       //----------------------------------------------------------------------
...
//   //           // and then get the url back and then
//   //           // save the url in imageUrl to save space

//   //           //----------------------------------------------------------------------
//   //           // 2/26/26 update
//   //           // Upload base64 image -> Firebase Storage, then store Firebase url in MongoDB
...
//   //             ) {
//   //               // const uploaded =
//   //               //   await uploadBase64ImageToFirebase(capturedImage);
//   //               // firebaseImagePath = uploaded; // mongodb stores the string url now
//   //               const uploaded = await uploadBase64ImageToFirebase(
//   //                 capturedImage,
//   //                 messageId,
//   //               );
...
//   //                     output: openAIResponse + lastError,
//   //                     imageURL: "",
//   //                     // imageURL: firebaseImagePath as string,
//   //                     location: {
//   //                       lat: coords?.latitude as number,
//   //                       lon: coords?.longitude as number,
//   //                     },
...
//   //           //----------------------------------------------------------------------
//   //           // 2/26/26 update
//   //           // Upload base64 image to Firebase Storage, then store Firebase url in MongoDB, not base64
//   //           let firebaseImagePath = capturedImage as string;
//   //           try {
//   //             if (
//   //               capturedImage &&
...
//   //             );
//   //           }
//   //           setUserInput(""); // added here (needs one more setUserInput)
//   //         }
//   //       } catch (error) {
//   //         console.error("Error creating chat log:", error);
//   //       }
